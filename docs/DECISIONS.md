...
**D3 — Feature-first structure with explicit controller / service / model layers.** One folder per bounded context — `quotes/` and `favorites/` — each split into `*.controller.ts` (HTTP: routing, request validation, status codes), `*.service.ts` (business logic seam), `*.model.ts` (domain types), plus the integration/persistence module (`favqs.client.ts`, `favorites.store.ts`). Cross-cutting pieces live outside features: `http/` (central error handling, later logging/metrics/correlation), `utils/` (pure helpers), `testing/` (test fixtures); `app.ts` composes services into controllers, `index.ts` wires real dependencies. Controllers never build error responses — they throw, and the central error middleware in `http/error-handler.ts` owns every error → status/shape mapping. Trade-off, named deliberately: at this size the services mostly delegate; they exist as the seam where business logic lands as the app grows, and the whole layout maps 1:1 to NestJS controllers/providers/modules.
...
**D14 — React Native Paper + layered mobile structure.** Paper gives accessible Material components (Card, Searchbar, Snackbar, ActivityIndicator) and built-in light/dark theming for one dependency. The mobile app mirrors the server's separation of concerns: `screens/` are thin controllers, `hooks/` hold the data/state logic (`useAsync`, `useLikeQuote`, `useFavorites`), `api/` is the HTTP client with normalized `ApiError`, `models/` the shared DTO types, `components/` pure presentational pieces, `navigation/` + `theme/` cross-cutting.

**D15 — Dislikes as a block-list, enforced in the service layer.** "Don't show this again" stores just the quote id (in-memory set, same swap-out interface pattern as favorites). The quotes *service* owns the rules — search results filtered, quote-of-the-day retried up to 3 times then 404 `NO_QUOTE_AVAILABLE`, dislike evicts any matching favorite — which is exactly the business logic the previously-thin service layer was waiting for. Trade-off: a determined FavQs rotation could still exhaust 3 retries; the cap keeps upstream calls bounded.
